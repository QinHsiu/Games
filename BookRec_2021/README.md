...
##### 提供的数据适合使用序列推荐模型进行建模，所以这里直接使用现有的序列推荐模型进行训练并作为基准模型在NDCG、HitRatio、F1-Score上进行比较进行比较，其结果如下所示，因为官方提供的baseline在使用验证集进行评估的时候是使用采样99个负样本加上正样本标签（共计100）来计算评价指标的所以其效果在验证集上明显好于基于序列的全排序模型，但是其在测试集上效果就很差。
